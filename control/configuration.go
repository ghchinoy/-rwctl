...
func ViperToConfiguration(configmap map[string]interface{}, debug bool) (Configuration, error) {
	...
	for k := range configmap {
		keys = append(keys,k)
		if k == "url" {
			config.URL = configmap["url"].(string)
		} else if k == "email" {
			config.Email = configmap["email"].(string)
		} else if k == "password" {
			config.Password = configmap["password"].(string)
		} else if k == "theme" {
			config.Theme = configmap["theme"].(string)
		} else if k == "console-username" {
			config.ConsoleUsername = configmap["console-username"].(string)
		} else if k == "loginDomainID" {
			config.ConsoleUsername = configmap["loginDomainID"].(string)
		}
	}
	if debug {
		log.Println("Config keys", strings.Join(keys, ", "))
	}
...
func AddCsrfHeader(req *http.Request, client *http.Client) *http.Request {
	...
	req.Header.Add("rwctl", version.Version())
	return req
}
